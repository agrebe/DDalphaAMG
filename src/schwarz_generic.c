...
void schwarz_PRECISION_alloc( schwarz_PRECISION_struct *s, level_struct *l ) {
  ...
  if ( g.method == 4 ) {
    ...
    fgmres_PRECISION_struct_alloc( 6, 1, (l->depth==0)?l->inner_vector_size:l->vector_size,
                                   EPS_PRECISION, _COARSE_GMRES, _NOTHING, NULL,
                                   (l->depth==0)?(g.odd_even?apply_schur_complement_PRECISION:d_plus_clover_PRECISION):
                                   (g.odd_even?coarse_apply_schur_complement_PRECISION:apply_coarse_operator_PRECISION),
                                   &(l->sp_PRECISION), l );
...
#ifndef EXTERNAL_DD
  if ( l->depth == 0 ) {
    MALLOC( s->oe_buf[0], complex_PRECISION, l->inner_vector_size );
  }
#endif
...
#ifndef EXTERNAL_DD
  if ( l->depth == 0 ) {
    FREE( s->oe_buf[0], complex_PRECISION, l->inner_vector_size );
    s->oe_buf[0] = NULL;
  }
#endif
